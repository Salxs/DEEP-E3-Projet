 ...
 * @post cette fonction effectue des mesures régulières avec un capteur HCSR04 placé sur les ports PB11 (Trig) et PB10 (Echo)
...
void HCSR04_demo_state_machine(GPIO_TypeDef * TRIG_GPIO, uint16_t TRIG_PIN, GPIO_TypeDef * ECHO_GPIO, uint16_t ECHO_PIN, uint16_t * distance_return)
{
	typedef enum
	{
		INIT,
	...
	{
		case INIT:
			//if(HCSR04_add(&id_sensor, GPIOB, GPIO_PIN_11, GPIOB, GPIO_PIN_10) != HAL_OK)
